...
class Employee extends CI_Controller
{
    ...
    public function employee_Update()
    {
        ...
        if (!empty($_FILES['em_photo'])) {
            if (!empty($xx->Employee_Pic_thum) && !empty($xx->Company_Logo_org)) {
                unlink("./uploads/employeePhoto_thum/" . $xx->Employee_Pic_thum);
                unlink("./uploads/employeePhoto_org/" . $xx->Company_Logo_org);
            }
            $file = $_FILES['em_photo']['name'];
            $uploaddir = 'uploads/employeePhoto_thum/'.$file; 
            $uploaddir1 = 'uploads/employeePhoto_org/'.$file; 
            move_uploaded_file( $_FILES['em_photo']['tmp_name'],$uploaddir);
            move_uploaded_file( $_FILES['em_photo']['tmp_name'],$uploaddir1);
            $this->db->query("update tbl_employee set Employee_Pic_thum = '$file', Employee_Pic_org='$file' where Employee_SlNo = '$id'");
        }
    ...
    public function deleteLeaveEntry()
    {
        ...
        try {
            $this->db->set(['status' => 'd'])->where(['leave_SlNo' => $leaveId->leave_id])->update('tbl_emp_leave');
    ...
    public function getLeaveReport()
    {
        ...
        $res = $this->db->query("
                select 
                    ...
                    u.FullName,  
                    b.Brunch_name,  
                    el.AddTime as addTime,
                    n.description as display_note,
                    us.User_Name as addbyusername
                from tbl_emp_leave el
                join tbl_user u on u.User_SlNo = el.User_ID
                left join tbl_user us on us.User_SlNo = el.AddBy
                left join tbl_leave_note n on n.Note_SlNo = el.note_id
                left join tbl_brunch b on b.brunch_id = u.userBrunch_id
                WHERE el.status != 'd'
                $clauses
            ")->result();
    ...
    public function updateStatusLeave()
    {
        ...
        $this->db->query("UPDATE `tbl_emp_leave` SET `ApproveBy`= ?
        WHERE `leave_SlNo`= ?", [$this->session->userdata("FullName"), $id]);
    ...
    public function search_attendence(){
        ...
        $clauses = "";
        if(isset($data->userId) && $data->userId != ''){
            $clauses .= " and tea.user_id = '$data->userId' ";
        }

        if(isset($data->brunchId) && $data->brunchId != '')
        {
            $clauses .= " and tea.branch_id = '$data->brunchId' ";
        }
        
        if(isset($data->dateFrom) && $data->dateFrom != '' && isset($data->dateTo) && $data->dateTo != '')
        {
            $clauses .= " and tea.date between '$data->dateFrom' and '$data->dateTo' ";
        }
