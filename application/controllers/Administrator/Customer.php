...
class Customer extends CI_Controller
{
    ...
    public function getCustomers()
    {
        ...
        $customers = $this->db->query("
            ...
            left join tbl_referances rf on rf.referance_id = c.referance_no
            where c.status = 'a'
    ...
    public function getCustomerDue()
    {
        ...
        if (isset($data->areaId) && $data->areaId != '') {
            $customerIdClause = " and c.area_ID = '$data->areaId'";
        }
        if (isset($data->referanceNo) && $data->referanceNo != '') {
            $customerIdClause = " and c.Referance_No = '$data->referanceNo'";
        }
        ...
        $dueResult = $this->db->query("
            ...
            (select ifnull(sum(s.ps_s_r_amount), 0.00) 
                from tbl_product_serial_numbers s where s.cus_id = c.Customer_SlNo) as returnedAmount,
            ...
            from tbl_customer c
            left join tbl_district d on d.District_SlNo = c.area_ID
            left join tbl_referances r on r.referance_id = c.Referance_No
    ...
    public function getCustomerDueAll()
    {
        $branchId = $this->session->userdata('BRANCHid');

        $allCustomer = $this->db->query("SELECT `Customer_SlNo` as id FROM `tbl_customer` WHERE `status` = 'a' and `Customer_brunchid` = ?", $branchId)->result();
        ...
        foreach ($allCustomer as $key => $value) {
            $result = $this->db->query("
                ...
                from tbl_customer c
                left join tbl_referances r on r.referance_id = c.Referance_No
                where c.Customer_SlNo = '$value->id'
            ")->row();

            if ($result->dueAmount > 0) {
                array_push($dueResult, $result);
            }
        }
    ...
    function getCustomerLedger()
    {
        ...
        $payments = $this->db->query("
            ...
            select
                ...
                0.00 as paid_out,
                0.00 as balance,
                cp.CPayment_invoice as reference
            from tbl_customer_payment cp
            left join tbl_bank_accounts ba on ba.account_id = cp.account_id
            ...
            select
                ...
                0.00 as paid_out,
                0.00 as balance,
                sr.SaleMaster_InvoiceNo as reference
