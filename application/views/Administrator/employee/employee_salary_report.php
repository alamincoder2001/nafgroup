...
<div id="salaryReport">
            ...
            <form class="form-inline" @submit.prevent="showReport">
                ...
                <div class="form-group" v-if="reportType == 'records'">
                    ...
                    <select class="form-control" style="min-width:200px;"
                        v-bind:style="{display: comEmployees.length > 0 ? 'none' : ''}"></select>
                    <v-select v-bind:options="comEmployees" v-model="selectedEmployee" label="display_text"
                        style="display:none" v-bind:style="{display: comEmployees.length > 0 ? '' : 'none'}"></v-select>
                </div>

                <div class="form-group">
                    <label>Month</label>
                    <select class="form-control" style="min-width:150px;"
                        v-bind:style="{display: months.length > 0 ? 'none' : ''}"></select>
                    <v-select v-bind:options="months" v-model="selectedMonth" label="month_name" style="display:none"
                        v-bind:style="{display: months.length > 0 ? '' : 'none'}"></v-select>
                </div>
    ...
    <div class="row" style="margin-top: 10px;display:none;"
        v-bind:style="{display: payments.length > 0 || paymentSummary.length > 0 ? '' : 'none'}">
        <div class="col-md-12">
        ...
        <div class="col-md-12">
            <div class="table-responsive" id="reportContent">
                <div style="display:none;"
                    v-bind:style="{display: payments.length > 0 && reportType == 'records' ? '' : 'none'}">
                        ...
                        <tfoot>
                            <tr style="font-weight:bold;">
                                ...
                                <td style="text-align:right;">
                                    {{ payments.reduce((prev, curr) => { return prev + parseFloat(curr.payment_amount)}, 0).toFixed(2) }}
                                </td>
                                <td style="text-align:right;">
                                    {{ payments.reduce((prev, curr) => { return prev + parseFloat(curr.deduction_amount)}, 0).toFixed(2) }}
                                </td>
                            </tr>
                        </tfoot>
                    </table>
                ...
                <div style="display:none;"
                    v-bind:style="{display: paymentSummary.length > 0 && reportType == 'summary' ? '' : 'none'}">
                    <h3 style="text-align:center;">Salary Summary</h3>
                    <h6 style="text-align:center;" v-if="selectedMonth != null">{{ selectedMonth.month_name }}</h6
                    ...
                    <table class="table table-bordered table-condensed">
                        ...
                        <tfoot>
                            <tr style="font-weight:bold;">
                                ...
                                <td style="text-align:right;">
                                    {{ paymentSummary.reduce((prev, curr) => { return prev + parseFloat(curr.salary_range)}, 0) }}
                                </td>
                                <td style="text-align:right;">
                                    {{ paymentSummary.reduce((prev, curr) => { return prev + parseFloat(curr.paid_amount)}, 0) }}
                                </td>
                                <td style="text-align:right;">
                                    {{ paymentSummary.reduce((prev, curr) => { return prev + parseFloat(curr.deducted_amount)}, 0) }}
                                </td>
                                <td style="text-align:right;">
                                    {{ paymentSummary.reduce((prev, curr) => { return prev + parseFloat(curr.due_amount)}, 0) }}
                                </td>
                            </tr>
                        </tfoot>
                    </table>
    ...
    computed: {
        comEmployees() {
            return this.employees.map(employee => {
                employee.display_text = employee.Employee_SlNo == '' ? employee.Employee_Name :
